...
1. **Check prerequisites**: run `vale --version` to confirm vale is available. If the command is not found, tell the user to install it by following the [Elastic vale linter guide](https://www.elastic.co/docs/contribute-docs/vale-linter#use-the-vale-linter-locally) before continuing.
...
- [ ] `vale` was confirmed available (or the user was sent to the install guide).
