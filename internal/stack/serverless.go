...
func (sp *serverlessProvider) createClients(project *serverless.Project, appConfig *install.ApplicationConfiguration) error {
	...
	logger.Tracef(">> Using package registry URL (TO BE REMOVED): %s", packageRegistryBaseURL(sp.profile, appConfig))
...
func (sp *serverlessProvider) startLocalServices(ctx context.Context, options Options, config Config) error {
	err := applyLocalResources(sp.profile, options.StackVersion, options.OverrideAgentVersion, config)
	if err != nil {
