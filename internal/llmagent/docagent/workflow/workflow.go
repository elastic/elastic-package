...
func buildInstructions() string {
	...
	sb.WriteString("11. Document ALL advanced settings with appropriate warnings (security, debug, SSL, etc.)\n")
	sb.WriteString("12. Use sentence case for headings (e.g., 'Vendor-side configuration' NOT 'Vendor-Side Configuration')\n")
	sb.WriteString("13. When showing example values like example.com, 10.0.0.1, or <placeholder>, add '(replace with your actual value)' or use format like `<your-hostname>`\n")
	sb.WriteString("14. Generate ONLY ONE H1 heading (the title) - all other headings should be H2 or lower\n")
	sb.WriteString("15. NEVER use # for code examples or configuration sections - use ### or #### instead\n")
	sb.WriteString("16. Heading levels must be sequential: H1 → H2 → H3 → H4 (never skip levels like H2 → H4)\n")
	sb.WriteString("17. In ## Troubleshooting, use Problem-Solution bullet format (NOT tables)\n")
	sb.WriteString("\n=== CONSISTENCY REQUIREMENTS ===\n")
	sb.WriteString("18. NEVER put bash comments (lines starting with #) outside code blocks - they will be parsed as H1 headings!\n")
	sb.WriteString("19. Use these EXACT subsection names in Troubleshooting:\n")
	...
	sb.WriteString("20. Use sentence case for ALL subsections (capitalize only first word): '### Vendor-specific issues' NOT '### Vendor-Specific Issues'\n")
	sb.WriteString("21. Under ## Reference, use:\n")
	...
	sb.WriteString("22. All code blocks MUST have language specified: ```bash, ```yaml, ```json - NEVER use bare ``` blocks\n")
	return sb.String()
}
