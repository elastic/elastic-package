		...
		if event.Content != nil {
			for _, part := range event.Content.Parts {
				...
				if part.FunctionResponse != nil {
					response := part.FunctionRespone

					logger.Debugf("Function response for: %s", response.Name)

					// Create tool response span
					_, toolSpan := tracing.StartToolSpan(ctx, response.Name+"_response", nil)

					// Format the response content
					var responseContent string
					if content, exists := response.Response["content"]; exists {
						responseContent = fmt.Sprintf("%v", content)
					} else if errContent, exists := response.Response["error"]; exists {
						responseContent = fmt.Sprintf("Error: %v", errContent)
					} else {
						// Marshal the entire response
						if respJSON, err := json.Marshal(response.Response); err == nil {
