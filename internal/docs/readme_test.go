...
func TestRenderReadmeWithSampleEvent(t *testing.T) {
	cases := []struct {
		...
		typedSampleEventsDataStream string // if set, typedSampleEvents are written under this data_stream/<name>/
		expected                    string
	}{
		{
...
` + "```",
		...
		{
			title:        "README with type-qualified sample events",
			packageRoot:  t.TempDir(),
			templatePath: "_dev/build/docs/README.md",
			readmeTemplateContents: `
{{- generatedHeader }}
...
# README
Introduction to the package
An example **logs** event looks as following:
...
An example **metrics** event looks as following:
...
` + "```",
		...
		{
			title:        "README with plain and type-qualified sample events in same data stream",
			packageRoot:  t.TempDir(),
			templatePath: "_dev/build/docs/README.md",
			readmeTemplateContents: `
{{- generatedHeader }}
...
# README
Introduction to the package
An example event for ` + "`mixed`" + ` looks as following:
...
` + "```",
			dataStreamName:              "mixed",
			sampleEventJsonContents:     `{"id":"plain"}`,
			typedSampleEvents:           map[string]string{"sample_event_logs.json": `{"id":"log1"}`},
			typedSampleEventsDataStream: "mixed",
		},
	}
	...
	for _, c := range cases {
		t.Run(c.title, func(t *testing.T) {
			...
			createReadmeTemplateFile(t, c.packageRoot, c.readmeTemplateContents)
			if c.sampleEventJsonContents != "" {
				createSampleEventFile(t, c.packageRoot, c.dataStreamName, c.sampleEventJsonContents)
			}
			for filename, contents := range c.typedSampleEvents {
				if c.typedSampleEventsDataStream != "" {
					createNamedSampleEventFileInDataStream(t, c.packageRoot, c.typedSampleEventsDataStream, filename, contents)
				} else {
					createNamedSampleEventFile(t, c.packageRoot, filename, contents)
				}
			}
...
func createNamedSampleEventFile(t *testing.T, packageRoot, filename, contents string) {
	t.Helper()
	err := os.WriteFile(filepath.Join(packageRoot, filename), []byte(contents), 0644)
	require.NoError(t, err)
}

func createNamedSampleEventFileInDataStream(t *testing.T, packageRoot, dataStreamName, filename, contents string) {
	t.Helper()
	dataStreamFolder := createDataStreamFolder(t, packageRoot, dataStreamName)
	err := os.WriteFile(filepath.Join(dataStreamFolder, filename), []byte(contents), 0644)
	require.NoError(t, err)
}
