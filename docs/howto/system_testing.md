...
| `TLS_DIR` | string | Path to the directory containing generated TLS certificates for mock services. See [On-disk TLS certificates](#on-disk-tls-certificates-for-mock-services). |
| `tls_cert` | helper | Generates a PEM-encoded TLS certificate for a given domain. See [TLS certificate helpers](#tls-certificate-helpers). |
| `tls_key` | helper | Generates a PEM-encoded private key matching `tls_cert` for the same domain. See [TLS certificate helpers](#tls-certificate-helpers). |
| `tls_ca` | helper | Emits the CA certificate PEM generated by the compose deployer. See [On-disk TLS certificates](#on-disk-tls-certificates-for-mock-services). |
...
Two Handlebars helpers are available for generating TLS certificates at test
time: `tls_cert` and `tls_key`. They produce PEM-encoded certificate and
private key material for a given domain name, removing the need for static
PEM blocks that expire and require manual rotation.
...
The domain argument sets the certificate's Common Name and Subject Alternative
Name. Use it when the test service checks the certificate against a specific
hostname:

```yaml
        {{{tls_cert "securityhub.xxxx.amazonaws.cn" indent=8}}}
```

The `indent` hash parameter controls whitespace prepended to continuation lines
(all lines after the first). Set it to match the column where the helper tag
appears in the template. This is needed because PEM blocks are multi-line and
YAML block scalars require consistent indentation.

Certificates generated by `tls_cert` and `tls_key` with the same domain within
a single config file are guaranteed to be a matching pair.

##### On-disk TLS certificates for mock services

When a mock service container needs its own TLS cert and key files (e.g.
`--tls-cert=/tls/server.crt`), `elastic-package` can generate them
automatically. If any service in the Docker Compose file declares a
`hostname:`, the compose deployer creates a CA and per-service certificate
before `docker compose up`.

The generated files are written to a `tls/` subdirectory under the test
output directory and exposed to compose via the `TLS_DIR` environment
variable:

| File | Description |
|---|---|
| `${TLS_DIR}/ca.crt` | CA certificate (PEM) |
| `${TLS_DIR}/<service-name>.crt` | Server certificate (full chain) |
| `${TLS_DIR}/<service-name>.key` | Server private key |

`<service-name>` is the compose service name (e.g. `mock_server`). Each
certificate includes dual SANs: the compose `hostname:` value and the
network alias `svc-<package>`.

Reference `TLS_DIR` in `docker-compose.yml` to volume-mount the certs:
...
services:
  mock_server:
    ...
    image: mock:latest
    command: --tls-cert=/tls/mock_server.crt --tls-key=/tls/mock_server.key
    volumes:
      - ${TLS_DIR}:/tls:ro
```

To inline the matching CA PEM in a test config (so the agent trusts the
mock server), use the `tls_ca` helper:
...
`tls_ca` takes no domain argument — there is a single shared CA per test
run. Like `tls_cert`/`tls_key`, it accepts an `indent=` parameter for
YAML alignment.

On `--no-provision` and `--tear-down` runs the deployer reads back the
existing `ca.crt` so `tls_ca` continues to work without regenerating
certificates.
