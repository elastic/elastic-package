...
- `required_inputs/with_linked_template_path` — same as above, but the integration-owned policy input template is supplied via `agent/input/owned.hbs.link` (see `docs/howto/dependency_management.md` for `template_path` vs `.link` naming).
...
3. Build and install in **dependency order** (input packages before integrations that require them). Examples:
   ...
   elastic-package build -C test/manual_packages/required_inputs/with_linked_template_path --zip
   ```
