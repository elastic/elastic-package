...
echo "      - label: \":go: Integration test: script-upgrade\""
echo "        command: ./.buildkite/scripts/integration_tests.sh -t test-script-upgrade"
echo "        agents:"
echo "          provider: \"gcp\""
echo "          image: \"${UBUNTU_X86_64_AGENT_IMAGE}\""
echo "        artifact_paths:"
echo "          - build/test-results/*.xml"
echo "          - build/elastic-stack-dump/script-upgrade/logs/*.log"
echo "          - build/elastic-stack-dump/script-upgrade/logs/fleet-server-internal/**/*"
echo "          - build/test-coverage/coverage-*.xml"
