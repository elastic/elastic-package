...
- **otelcol input packages**: `data_stream.dataset` is the **base** dataset name (without a `.otel` suffix). Elastic Agent appends `.otel` at ingest; Fleet does not add it in the policy payload. System tests in elastic-package mirror that append when resolving data stream names and expected document datasets—do not put `.otel` in the override unless you intentionally want a `*.otel.otel` dataset in Elasticsearch.
