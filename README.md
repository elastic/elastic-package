...
- Related to verifying packages downloaded from the Elastic Package Registry (EPR):
    ...
    - `ELASTIC_PACKAGE_VERIFIER_PUBLIC_KEYFILE`: Path to an armored OpenPGP public key
      used to verify EPR package signatures. When set, it takes precedence over the
      embedded Elastic key. Use this to verify packages signed with a rotated key
      without upgrading `elastic-package`.
    ...
    and set `ELASTIC_PACKAGE_VERIFIER_PUBLIC_KEYFILE` to its path.
