...
func testRunnerAssetCommandAction(cmd *cobra.Command, args []string) error {
	...
	results, err := launcher.Run(ctx, testType, testrunner.TestOptions{
		Profile:                    profile,
...
func testRunnerSystemCommandAction(cmd *cobra.Command, args []string) error {
	...
	runner := system.NewSystemRunner(runnerOptions)
	...
	if multiErr != nil {
		return fmt.Errorf("error running package %s tests: %w", testType, multiErr)
	}
